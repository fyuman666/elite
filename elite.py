...
def handle_start(message):
    ...
    response_message = '''подпишитесь чтобы использовать бота на канал создателя проекта Komary Clicker @fyuman_guard.'''
    bot.send_message(chat_id, response_message)
    show_main_menu(chat_id)

def show_main_menu(chat_id):
    keyboard = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    button_get_money = types.KeyboardButton(text='💸 click 0.03₽')
...
@bot.message_handler(func=lambda message: message.text == '💸 click 0.03₽')
def handle_get_money(message):
    chat_id = message.chat.id
    if is_user_banned(chat_id):
        bot.send_message(chat_id, '📕 Ваша учетная запись заблокирована администратором!')
        return
    amount = 0.03
